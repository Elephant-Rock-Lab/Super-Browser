...
class TestNewInjectVectors:
    """Gate 3 — v1.11.0 new inject vectors (Connection, SAB, Touch, Orientation, Gamut)."""

    def test_connection_api_inject(self) -> None:
        """Connection API override appears in generated JS."""
        matrix = _make_matrix(
            connection_effective_type="4g",
            connection_downlink=5.5,
            connection_rtt=100,
            connection_save_data=True,
        )
        js = generate_inject(matrix)
        assert "Connection API override" in js
        assert "effectiveType" in js
        assert '"4g"' in js

    def test_shared_array_buffer_hidden(self) -> None:
        """SAB is deleted when has_shared_array_buffer=False."""
        matrix = _make_matrix(has_shared_array_buffer=False)
        js = generate_inject(matrix)
        assert "delete window.SharedArrayBuffer" in js

    def test_shared_array_buffer_kept(self) -> None:
        """SAB delete block is guarded by false condition when has_shared_array_buffer=True."""
        matrix = _make_matrix(has_shared_array_buffer=True)
        js = generate_inject(matrix)
        # The template emits "if (false === false)" which is falsy � verify the guard condition.
        assert "if (true === false)" in js

    def test_touch_events_removed_when_no_touch(self) -> None:
        """Touch event constructors are deleted when touch_support=False."""
        matrix = _make_matrix(touch_support=False)
        js = generate_inject(matrix)
        assert "delete window.TouchEvent" in js
        assert "delete window.Touch" in js

    def test_touch_events_kept_when_touch(self) -> None:
        """Touch delete block is guarded by false condition when touch_support=True."""
        matrix = _make_matrix(touch_support=True, navigator_max_touch_points=5)
        js = generate_inject(matrix)
        # The template emits "if (true === false)" which is falsy � verify the guard condition.
        assert "if (true === false)" in js

    def test_screen_orientation_override(self) -> None:
        """Screen orientation type and angle are overridden."""
        matrix = _make_matrix(
        ...
        assert '"portrait-primary"' in js

    def test_color_gamut_override(self) -> None:
        """matchMedia responses are overridden for color-gamut queries."""
        matrix = _make_matrix(color_gamut="p3")
        js = generate_inject(matrix)
        assert "color-gamut" in js
        assert "dynamic-range" in js
        assert '"p3"' in js

    def test_all_new_vectors_together(self) -> None:
        """All 5 new vectors coexist in the same payload."""
        matrix = _make_matrix(
            touch_support=True,
            color_gamut="rec2020",
            has_shared_array_buffer=False,
            connection_effective_type="3g",
            screen_orientation_type="landscape-secondary",
            screen_orientation_angle=180,
        )
        js = generate_inject(matrix)
        assert "Connection API" in js
        assert "SharedArrayBuffer" in js
        assert "Touch events" in js
        assert "screen.orientation" in js
        assert "color-gamut" in js

    def test_new_matrix_fields_defaults(self) -> None:
        """New matrix fields have correct defaults."""

        m = _make_matrix()
        assert m.touch_support is False
        assert m.color_gamut == "srgb"
        assert m.has_shared_array_buffer is True

    def test_derive_defaults_touch_from_max_touch_points(self) -> None:
        """derive_matrix defaults touch_support from maxTouchPoints > 0."""
        from super_browser.stealth.consistency.derive import _dict_to_matrix
        # Direct dict-based construction tests the default logic
        m = _dict_to_matrix({"navigator_max_touch_points": 5, "seed": "test"})
        assert m.touch_support is True

        m2 = _dict_to_matrix({"navigator_max_touch_points": 0, "seed": "test"})
        assert m2.touch_support is False
