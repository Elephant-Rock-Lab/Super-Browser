...
superbrowser-mcp --allow-actions       # 31 tools (adds the action tier)
python -m super_browser.mcp_server
```

Both start a **stdio** server. The default server advertises 19 tools and
...
| `extract_image_text` | `selector` (optional), `bounds` (optional), `full_page` (optional), `language` (optional, default `"eng"`), `min_confidence` (optional, 0.0–1.0) | OCR text extraction from screenshots. Reads text embedded in images (flyer/catalog sites, product photos) that `extract_text` and `observe` cannot see. Returns joined `text` + structured `words` (`{text, x, y, w, h, confidence}`). `selector` and `bounds` are mutually exclusive; no args = viewport OCR. Requires the `[vision]` extra (pytesseract + Pillow) and a system Tesseract binary; returns `ocr_unavailable` error if not installed. Redaction covers both `text` and `words[].text`. | Yes |
